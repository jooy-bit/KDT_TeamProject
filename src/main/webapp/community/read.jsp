	...
	<!-- 아래 ul id는 추후 변경 가능  -->
		<ul id="table24">
			...
			<li>
				<ul class="mrow4">
					<li>작성날짜</li>
					<li><fmt:formatDate value="${vo.crt_date }" type="both"/></li>
					<!-- pattern="yyyy-MM-dd HH:mm:ss , type= date,time,both -->
					<li>조회수</li>
					<li><c:out value="${vo.read_cnt}"/></li>
				</ul>
			</li>
			<li id="content">
				<ul>
...
style="background-color:#f3f3f3;font-size:inherit;resize: none;border:none;"><c:out value="${vo.contents}"/></textarea>
					</li>				
				</ul>
			</li>
		</ul>
	<div style="text-align: center;margin-bottom: 10px;">
	<c:if test="${user.id==vo.writer_id }">  <!-- session 에 저장된 user애트리뷰트의 id와 작성자의 id 가 같은면 보이기 -->
	...
	<input type="hidden" name="writer" class="input" value="${user.id }">
...
<!-- 저장버튼 테스트를 위해 변경 --><c:if test="${sessionScope.user != null }">  <!-- 구현 완료 : 로그인 했을때 -->
			...
			<li>
					<span>댓글</span>
					<span>[<c:out value="${vo.comment_cnt }" />]</span> <!-- 댓글갯수 -->
				<hr>
			</li>
			...
			<li>
				<ul class="crow">
					<li><c:out value="${cmt.writer_id }" /></li>								
					<li><c:out value="${cmt.crt_date }" /></li>	
				<c:if test="${user.id==cmt.writer_id }">  <!-- session 에 저장된 user애트리뷰트의 id와 작성자의 id 가 같은면 보이기 -->		
